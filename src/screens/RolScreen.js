...
import RolDialog from '../components/RolDialog';
...
export default function RolScreen() {  
  ...
  const getAllData = async () => {
    try {
      const apexApiCall = await fetchCustom('GetMasters/rol/All');
  ...
  const delMenu = async (id) => {
    try {             
      const apexApiCall = await fetchCustom(`DelMasters/${id}/rol`,undefined,"DELETE");
      const dataJson    = await apexApiCall.json();     
      if(dataJson['status']){                    
        getAllData();          
      }      
  ...
  return (
      <>   
        ...
        {(OpenDialog) && <RolDialog isOpen={OpenDialog} handleClose={handleCloseDialog} DataMaster={DataMaster} />}
        <Paper >                         
            <ToolbarTable />
            <TableContainer>
